...
<script src='https://cdnjs.cloudflare.com/ajax/libs/jscolor/2.0.4/jscolor.min.js'></script>
  ...
  <p><button onclick="sendCommand(red)">Red</button><button onclick="sendCommand(green)">Green</button><button onclick="sendCommand(blue)">Blue</button>
  <p><button onclick="sendCommand(white)">White</button><button onclick="sendCommand(fullBrightness)">100% Brightness</button><button onclick="sendCommand(halfBrightness)">50% Brightness</button>
  <p><button onclick="sendCommand(off)">Turn Off</button><button onclick="sendCommand(reset)">Reset Settings</button>
  <script>
	var off = '{"command":"off"}'
    var red = '{"command":"set_rgb","r":255,"g":0,"b":0}'
    var white = '{"command":"set_rgb","r":255,"g":255,"b":255}'
    ...
    var reset = '{"command":"reset_settings"}'
